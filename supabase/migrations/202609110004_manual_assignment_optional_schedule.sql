-- Manual admin complaints do not require room-access data or an appointment.
-- Google Form complaints preserve the access-dependent scheduling workflow.
...
begin
  ...
  if v_complaint.source<>'manual' and v_access not in ('yes','no') then
  ...
  if v_complaint.source<>'manual' and v_access='no' and p_appointment_date is null then
