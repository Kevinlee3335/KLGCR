-- KLGCR Phase 4: material requests, inventory and issue history.
...
revoke insert,update,delete on public.material_requests, public.material_request_items, public.inventory_issue_history from authenticated;
grant select on public.inventory_items, public.material_requests, public.material_request_items, public.inventory_issue_history to authenticated;
...
begin
  ...
  if jsonb_array_length(coalesce(p_items,'[]'::jsonb))=0 then raise exception 'At least one material is required'; end if;
...
grant execute on function public.create_material_request(uuid,text,jsonb), public.review_material_request(uuid,boolean,text), public.issue_material_request(uuid) to authenticated;
