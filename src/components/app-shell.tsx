...
const adminNav = [["Dashboard", "/admin"], ["New Complaints", "/admin/complaints"], ["Maintenance Jobs", "/admin/jobs"], ["Daily Tasks", "#"], ["Material Requests", "#"], ["Inventory", "#"], ["Reports", "#"], ["Notifications", "#"], ["Users", "/admin/users"], ["Settings", "#"]];
const staffNav = [["My Dashboard", "/staff"], ["My Tasks", "/staff/tasks"], ["Material Request", "/staff/tasks?status=pending_material"], ["Monitoring", "/staff/monitoring"], ["Appointments", "#"], ["Completed Jobs", "/staff/tasks?status=completed"]];
...
export function AppShell({ profile, children, title }: { profile: Profile; children: React.ReactNode; title: string }) {
  ...
  return <div className="shell"><aside className="sidebar"><Brand/><nav className="nav" aria-label="Main navigation">{nav.map(([item, href]) => <a key={item} href={href}>{item}</a>)}</nav><div className="sidebar-footer"><strong>{profile.full_name}</strong><br/><small>{roleLabel[profile.role]}</small><LogoutForm/></div></aside><main className="main"><header className="topbar"><h1>{title}</h1><span className="badge">{profile.blocks?.map((block) => `Block ${block.code}`).join(" · ") || "All blocks"}</span></header><div className="content">{children}</div></main><nav className="mobile-nav" aria-label="Mobile navigation"><a href={isStaff ? "/staff" : "/admin"}><NavIcon kind="dashboard"/><span>Dashboard</span></a><a href={isStaff ? "/staff/tasks" : "/admin/complaints"}><NavIcon kind="tasks"/><span>Tasks</span></a>{!isStaff ? <a href="/admin/users"><NavIcon kind="users"/><span>Users</span></a> : <a href="/staff/tasks?status=pending_material"><NavIcon kind="more"/><span>Materials</span></a>}<a href={isStaff ? "/staff/monitoring" : "#"}><NavIcon kind="more"/><span>{isStaff ? "Monitoring" : "More"}</span></a></nav></div>;
}
