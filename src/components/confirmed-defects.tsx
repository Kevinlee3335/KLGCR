...
export function ConfirmedDefects({complaintId,defects}:{complaintId:string;defects:Defect[]}) {
 const [selected,setSelected]=useState(defectCatalog[0]); const [issue,setIssue]=useState(selected.issues[0]); const [commonArea,setCommonArea]=useState("");
 ...
 return <div className="confirmed-defects"><h3>Admin Confirmed Defects</h3><p className="subtle">Keep the student&apos;s original report above. Add the actual work items that Maintenance must handle.</p>
 ...
 {issue==="Other"&&<label className="field field-wide"><span>Other / Specify issue *</span><input name="otherIssue" required placeholder="Describe the issue"/></label>}{selected.group==="Common Area"&&<label className="field"><span>Common Area *</span><select name="commonArea" value={commonArea} onChange={e=>setCommonArea(e.target.value)} required><option value="">Select area</option>{commonAreas.map(area=><option key={area}>{area}</option>)}</select></label>}
 <label className="field"><span>{selected.group==="Common Area"?"Exact Location / Landmark":"Exact Location / Detailed Location"}</span><input name="exactLocation" placeholder="e.g. Near window / above study table"/></label>
 <div className="field-wide"><button className="button">Add confirmed defect</button></div></form></div>;
}