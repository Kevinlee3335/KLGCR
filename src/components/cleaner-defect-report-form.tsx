  ...
  return (
    <form action={createCleanerDefectReport} className="panel cleaner-report-page-form">
        ...
        {locationType === "room" ? (
          ...
          <label className="field"><span>Common area *</span>
            <select name="commonArea" required value={commonArea} onChange={(event) => setCommonArea(event.target.value)}>
              ...
              <option>Corridor</option><option>Balcony</option><option>Lobby</option><option>Common Bathroom</option>
              <option>Staircase</option><option>Drying Area</option><option>Visitor Room</option><option>Utility Room</option><option>Pantry</option><option>Other</option>
            </select>
          </label>
        )}
      ...
      <div className="cleaner-report-builder">
        ...
        <CheckoutDefectBuilder fieldName="defectsJson" emptyMessage="Add at least one defect to submit this report." />
      </div>
