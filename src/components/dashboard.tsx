...
function AdminDashboard({ name, data }: Omit<AdminDashboardProps, "kind">) {
  ...
  const kpis = [
    { title: "New Complaints", value: data.kpis.newComplaints, href: "/admin/complaints?status=new", icon: ShieldAlert, tone: "blue" },
    { title: "Today's Jobs", value: data.kpis.todayJobs, href: "/admin/jobs?scope=today-active", icon: ClipboardList, tone: "gold" },
    { title: "In Progress", value: data.kpis.inProgress, href: "/admin/jobs?status=in_progress", icon: PlayCircle, tone: "violet" },
    { title: "Pending Material", value: data.kpis.pendingMaterial, href: "/admin/jobs?status=pending_material", icon: PackageOpen, tone: "amber" },
    { title: "Under Monitoring", value: data.kpis.underMonitoring, href: "/admin/jobs?status=under_monitoring", icon: Clock3, tone: "purple" },
    { title: "Completed Today", value: data.kpis.completedToday, href: "/admin/jobs?scope=completed-today", icon: CheckCircle2, tone: "green" },
  ];
  const quickActions = [
  ...
  return <div className="admin-dashboard">
    ...
    <section className="admin-kpis" aria-label="Operational summary">{kpis.map(({ icon: Icon, ...kpi }) => <Link className={`kpi-card kpi-${kpi.tone}`} href={kpi.href} key={kpi.title}><span className="kpi-icon"><Icon size={20}/></span><div><span className="kpi-title">{kpi.title}</span><strong>{kpi.value}</strong></div><ArrowRight className="kpi-arrow" size={16}/></Link>)}</section>
    ...
    <div className="operations-grid">
      ...
      <section className="dashboard-card inventory-alerts"><div className="dashboard-section-title"><div><span>Inventory Alerts</span><small>Stock requiring attention</small></div><Link href="/admin/inventory">View all <ArrowRight size={14}/></Link></div><p className="inventory-group-label">Need To Order</p><div className="inventory-stats"><div><span className="alert-icon warning"><PackageOpen size={21}/></span><span><small>Near Reorder</small><strong>{data.inventory.nearReorder}</strong></span></div><div><span className="alert-icon danger"><AlertTriangle size={21}/></span><span><small>Out Of Stock</small><strong>{data.inventory.outOfStock}</strong></span></div></div></section>
    </div>
  </div>;
}
