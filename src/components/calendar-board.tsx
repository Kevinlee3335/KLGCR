...
export function CalendarBoard({ events, admin, form }: { events: CalendarEvent[]; admin?: boolean; form: ReactNode }) {
  ...
  const eventsByDay = useMemo(() => { const map = new Map<string, CalendarEvent[]>(); for (const event of events) { const day = isoDate(new Date(event.starts_at)); map.set(day, [...(map.get(day) || []), event]); } return map; }, [events]);
  ...
  return <div className="phone-calendar"><div className="calendar-toolbar"><div><p className="eyebrow">Johor · Malaysia</p><h2>{title}</h2><p className="subtle">Public holidays, team schedules and your own work.</p></div><div className="calendar-controls"><button className="button secondary calendar-today" type="button" onClick={() => setMonth(firstOfMonth(new Date()))}>Today</button><button className="calendar-arrow" type="button" aria-label="Previous month" onClick={() => shift(-1)}><ChevronLeft size={20}/></button><button className="calendar-arrow" type="button" aria-label="Next month" onClick={() => shift(1)}><ChevronRight size={20}/></button>{admin && <button className="button calendar-add" type="button" onClick={() => setShowForm((open) => !open)}><Plus size={18}/>{showForm ? "Close" : "Add schedule"}</button>}</div></div>{admin && showForm && <section className="calendar-create panel"><h3>Create schedule</h3><p className="subtle">Choose one employee or everyone.</p>{form}</section>}<section className="calendar-grid" aria-label={`${title} calendar`}><div className="calendar-weekdays">{weekday.map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-days">{dates.map((day) => { const date = isoDate(day); const dayEvents = eventsByDay.get(date) || []; const holiday = holidays.get(date); const outside = day.getMonth() !== month.getMonth(); return <div className={`calendar-day${outside ? " outside" : ""}${date === today ? " is-today" : ""}`} key={date}><div className="calendar-date"><span>{day.getDate()}</span>{holiday && <small>Holiday</small>}</div>{holiday && <div className="calendar-holiday" title={holiday}>{holiday}</div>}<div className="calendar-events">{dayEvents.slice(0, 3).map((event) => <div className={`calendar-event-pill${event.audience === "all_staff" ? " all" : ""}`} title={`${time.format(new Date(event.starts_at))} · ${event.title}`} key={event.id}><span>{time.format(new Date(event.starts_at))}</span>{event.title}</div>)}{dayEvents.length > 3 && <small className="calendar-more">+{dayEvents.length - 3} more</small>}</div></div>; })}</div></section>{!admin && <section className="panel calendar-personal"><div><h3>Add my reminder</h3><p className="subtle">Only you and Admin can see reminders you add yourself.</p></div>{form}</section>}<div className="calendar-legend"><span><i className="holiday-dot"/>Johor public holiday</span><span><i className="event-dot"/>Your schedule</span><span><i className="event-dot all"/>All employees</span></div></div>;
}