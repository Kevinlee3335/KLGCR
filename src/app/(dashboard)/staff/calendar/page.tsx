...
type CalendarEvent = { id: string; title: string; notes: string | null; starts_at: string; ends_at: string | null; event_type: "work" | "leave" | "no_leave" | "meeting" | "other"; staff: { full_name: string } | null; subject: { full_name: string } | null; };
...
export default async function StaffCalendar({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  const [appointmentResult, eventResult] = await Promise.all([
    ...
    supabase.from("calendar_events").select("id,title,notes,starts_at,ends_at,event_type,staff:profiles!calendar_events_assigned_to_fkey(full_name),subject:profiles!calendar_events_subject_staff_id_fkey(full_name)").gte("starts_at", `${start}T00:00:00+08:00`).lt("starts_at", `${next}T00:00:00+08:00`).order("starts_at"),
  ]);
