...
type CalendarEvent = { id: string; title: string; notes: string | null; starts_at: string; event_type: "work" | "leave" | "no_leave" | "meeting" | "other"; staff: { full_name: string } | null; };
...
export default async function StaffCalendar({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  const [appointmentResult, eventResult] = await Promise.all([
    ...
    supabase.from("calendar_events").select("id,title,notes,starts_at,event_type,staff:profiles!calendar_events_assigned_to_fkey(full_name)").gte("starts_at", `${start}T00:00:00+08:00`).lt("starts_at", `${next}T00:00:00+08:00`).order("starts_at"),
  ]);
  ...
  const items: CalendarItem[] = [
    ...events.map((event) => ({ id: `event-${event.id}`, date: malaysiaDate(event.starts_at), time: malaysiaTime(event.starts_at), label: event.title, detail: `${eventLabel[event.event_type]}${event.staff ? ` · ${event.staff.full_name}` : ""}${event.notes ? ` · ${event.notes}` : ""}`, href: null, tone: `calendar-${event.event_type}` })),
