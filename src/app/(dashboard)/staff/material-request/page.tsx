...
export default async function StaffMaterialRequestPage({ searchParams }: { searchParams: Promise<{ job?: string; error?: string; success?: string }> }) {
  ...
  return <AppShell profile={profile} title="Material Request">
    <div className="section-head"><div><p className="eyebrow">Phase 4</p><h2>Request Materials</h2><p className="subtle">Select a current job and up to three materials in one request.</p></div></div>
    ...
    <section className="panel">
      <form action="/staff/material-request/submit" method="post" className="form-grid">
        ...
        {[1,2,3].map((slot) => <div className="field-wide form-grid" key={slot}><label className="field-wide"><span>Material {slot}{slot > 1 ? " (optional)" : ""}</span><select name="itemId" required={slot === 1}><option value="">Select material</option>{materialOptions}</select></label><label><span>Quantity</span><input name="qty" type="number" min="0.01" step="0.01" required={slot === 1}/></label></div>)}
        <label className="field-wide"><span>Note</span><textarea name="note" rows={3} placeholder="Why these materials are needed"/></label>
    ...
    <section className="panel list-panel">{(requests || []).length === 0 ? <p className="subtle">No material requests yet.</p> : (requests || []).map((request: any) => <article className="list-row" key={request.id}><div><div className="actions"><strong>{request.request_no}</strong><span className="badge">{request.status}</span></div><p>{request.job?.job_no} · {request.job?.room_no}</p>{(request.items || []).map((row: any, index: number) => <p key={`${request.id}-${index}`}>{row.item?.item_code} · {row.item?.description} — Requested {row.requested_qty} {row.item?.unit || ""}{row.issued_qty ? ` · Issued ${row.issued_qty}` : ""}</p>)}{request.note && <small>{request.note}</small>}{request.rejection_reason && <p className="error">Rejected: {request.rejection_reason}</p>}{request.status === "issued" && request.job?.status === "pending_material" && <form action={`/staff/jobs/${request.job.id}/resume`} method="post"><button className="button" type="submit">Material Received · Resume Job</button></form>}</div></article>)}</section>
