...
export default async function StaffPage() {
  ...
  return <AppShell profile={profile} title="My Dashboard"><Dashboard kind="staff" name={profile.full_name} blocks={blocks} values={[count("assigned")+count("accepted")+count("reopened"), count("in_progress")+count("paused"), count("pending_material"), count("completed")+count("verified")+count("closed")]}/><div className="section-head tasks-head"><h2>Recent jobs</h2><Link className="text-link" href="/staff/tasks">View current tasks</Link></div><section className="panel list-panel">{recentResult.error ? <p className="error">{recentResult.error.message}</p> : <JobList rows={rows} staff compact/>}</section></AppShell>;
}
