...
const successMessages: Record<string, string> = { started: "Job started successfully.", completed: "Job completed successfully.", monitoring: "Monitoring details saved.", "pending-material": "Job marked as Pending Material.", resumed: "Job returned to In Progress.", "tenant-not-available": "Tenant not available recorded. The job remains open." };

export default async function StaffJob({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; success?: string; from?: string }> }) {
  ...
  const returnTo = staffJobReturnTo(query.from, job.status);
