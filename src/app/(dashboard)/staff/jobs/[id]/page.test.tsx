...
const job = {
  ...
  complaint: { id: "complaint-id", complaint_no: "CMP-1", reporter_name: "Resident Name", reporter_phone: "0199999999", complainant_name: null, complainant_contact: null, availability_date: "2026-09-12", availability_time: "10:00:00", room_access_permission: "YES" },
};
...
describe("Staff Job Detail", () => {
  ...
  it("shows complaint reporter data without exposing email", async () => {
    ...
    expect(projection).toContain("maintenance_jobs_complaint_id_fkey"); expect(projection).toContain("availability_date,availability_time"); expect(projection).not.toContain("preferred_date"); expect(projection).not.toContain("preferred_time"); expect(projection).not.toContain("reporter_email");
  });
  ...
  it("surfaces query errors instead of treating them as a missing job", async () => {
    queries.set("maintenance_jobs", query({ data: null, error: { message: "column does not exist" } }));
    await expect(StaffJob({ params: Promise.resolve({ id: job.id }), searchParams: Promise.resolve({}) })).rejects.toThrow("Unable to load staff job job-id: column does not exist");
    expect(notFoundMock).not.toHaveBeenCalled();
  });

  it("uses notFound only when the job query succeeds without a row", async () => {
    queries.set("maintenance_jobs", query({ data: null, error: null }));
    await expect(StaffJob({ params: Promise.resolve({ id: job.id }), searchParams: Promise.resolve({}) })).rejects.toThrow("NEXT_NOT_FOUND");
