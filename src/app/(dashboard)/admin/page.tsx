...
export default async function AdminPage() {
  ...
  const data: AdminDashboardData = {
    kpis: { newComplaints: Number(counts?.new_complaints ?? 0), todayJobs: Number(counts?.today_tasks ?? 0), inProgress: jobs.in_progress, pendingMaterial: jobs.pending_material, underMonitoring: jobs.under_monitoring, completedToday: Number(counts?.completed_today ?? 0),todayAppointments:todayAppointments||0,pendingAppointments:pendingAppointments||0,needAppointments:needAppointments||0 },
