...
export default async function AdminPage() {
  ...
  const [{ data: countRows, error: countError }, { count: assignedCount }, { count: inProgressCount }, { count: pendingMaterialCount }, { count: monitoringCount }, { count: completedCount }, { data: complaints }, { data: tasks }, { data: inventory },{count:todayAppointments},{count:pendingAppointments},{data:noShowRows,error:noShowError}] = await Promise.all([
    ...
    supabase.from("maintenance_jobs").select("*", { count: "exact", head: true }).eq("status", "under_monitoring"),
    ...
    supabase.from("appointments")
      ...
      .eq("status","no_show")
      .order("attended_at",{ascending:false}).limit(100),
  ]);
  ...
  const jobs = {
    ...
    pending_material: pendingMaterialCount || 0,
    under_monitoring: monitoringCount || 0,
