...
export default async function AdminPage() {
  ...
  const [{ data: countRows, error: countError }, { data: statusRows }, { data: complaints }, { data: tasks }, { data: inventory }] = await Promise.all([
    ...
    supabase.from("maintenance_jobs").select("status").in("status", ["assigned", "accepted", "in_progress", "paused", "pending_material", "under_monitoring", "completed", "verified", "closed", "reopened"]),
    ...
    supabase.from("maintenance_jobs").select("id,job_no,room_no,description,status,scheduled_for,assigned_at,block:blocks!block_id(code),assignee:profiles!assigned_to(full_name)").eq("scheduled_for", today).not("status", "in", '("completed","verified","closed","cancelled")').order("assigned_at", { ascending: true }).limit(5),
  ...
  for (const row of statusRows ?? []) { const bucket = ["accepted", "reopened"].includes(row.status) ? "assigned" : row.status === "paused" ? "in_progress" : ["verified", "closed"].includes(row.status) ? "completed" : row.status; if (bucket in jobs) jobs[bucket as keyof typeof jobs]++; }
