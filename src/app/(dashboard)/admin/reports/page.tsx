...
export default async function ReportsPage() {
  ...
  return <AppShell profile={profile} title="Reports">
    ...
    <div className="panel" style={{marginTop:18}}><div className="section-head"><h3>Report History</h3><div className="actions"><a className="button secondary button-link" href="/admin/reports/export?format=csv">CSV</a><a className="button secondary button-link" href="/admin/reports/export?format=xls">Excel</a></div></div>
      {!reports?.length ? <div className="empty"><div><strong>No reports yet</strong>Generate one now or wait for the automatic schedule.</div></div> : <div className="mobile-cards" style={{display:"grid"}}>{reports.map((r) => <article className="panel record-card" key={r.id}><div className="record-head"><strong>{labels[r.report_type] ?? r.report_type} · {r.block_group}</strong><span className="badge">{r.source}</span></div><div className="record-meta"><span>{r.report_date}</span><span>{new Date(r.created_at).toLocaleString("en-MY",{timeZone:"Asia/Kuala_Lumpur"})}</span></div><pre style={{whiteSpace:"pre-wrap",fontFamily:"inherit",lineHeight:1.5,marginTop:16}}>{r.whatsapp_text}</pre><button className="button secondary" type="button" onClick={undefined} disabled>Copy text from this box</button></article>)}</div>}
    </div>
  </AppShell>;
}
