...
export async function GET(request: Request) {
  ...
  const date=params.get("date")??"", block=params.get("block")??"", reportType=params.get("reportType")??"", staff=params.get("staff")??"";
  ...
  if(dataset==="material_usage" || dataset==="consumption"){
    let query=supabase.from("inventory_issue_history").select("id,qty,issued_at,staff_id,inventory_items(item_code,description,unit),maintenance_jobs(job_no,room_no,blocks(code)),profiles!inventory_issue_history_staff_id_fkey(full_name)").order("issued_at",{ascending:false}).limit(1000);
    ...
    if(dataset==="consumption"){
      ...
      for(const r of filtered as any[]){const code=r.inventory_items?.item_code??"Unknown";const cur=totals.get(code)??{description:r.inventory_items?.description??"",qty:0,unit:r.inventory_items?.unit??""};cur.qty+=Number(r.qty??0);totals.set(code,cur);}
      const rows=[...totals.entries()].sort((a,b)=>b[1].qty-a[1].qty).map(([code,v])=>[code,v.description,v.qty,v.unit]);
      return download(rows,["Item Code","Description","Total Used","Unit"],format,"KLGCR-Consumption-Analysis");
    }
    const rows=(filtered as any[]).map(r=>[r.issued_at,r.inventory_items?.item_code,r.inventory_items?.description,r.qty,r.inventory_items?.unit,r.maintenance_jobs?.job_no,r.maintenance_jobs?.blocks?.code,r.maintenance_jobs?.room_no,r.profiles?.full_name]);
  ...
  if(dataset==="inventory_transactions"){
    let query=supabase.from("inventory_adjustment_history").select("adjusted_at,adjustment_type,qty,balance_before,balance_after,note,inventory_items(item_code,description,unit)").order("adjusted_at",{ascending:false}).limit(1000);
    if(date){const [from,to]=malaysiaDayRange(date);query=query.gte("adjusted_at",from).lte("adjusted_at",to);}
    const {data,error}=await query;if(error)return new Response(error.message,{status:500});
    const rows=(data??[] as any[]).map(r=>[r.adjusted_at,r.inventory_items?.item_code,r.inventory_items?.description,r.adjustment_type,r.qty,r.inventory_items?.unit,r.balance_before,r.balance_after,r.note]);
  ...
  let query=supabase.from("report_snapshots").select("report_date,report_type,block_group,source,created_at,whatsapp_text").order("created_at",{ascending:false}).limit(500);
  if(date)query=query.eq("report_date",date);if(block)query=query.eq("block_group",block);if(reportType)query=query.eq("report_type",reportType);
  const {data,error}=await query;if(error)return new Response(error.message,{status:500});
  const rows=(data??[]).map(r=>[r.report_date,r.report_type,r.block_group,r.source,r.created_at,r.whatsapp_text]);
