...
export default async function ComplaintDetail({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<{error?:string;saved?:string;appointment?:string}> }) {
  ...
  <section className="panel detail-grid" style={{marginBottom:18}}><div className="field-wide"><h3>Student Availability</h3><p className="subtle">The preferred date and time do not schedule the visit. The administrator chooses the final appointment date and time.</p></div><div><span>Student Name</span><strong>{complaint.complainant_name||"—"}</strong></div><div><span>Phone</span><strong>{extra.reporter_phone||complaint.complainant_contact||"—"}</strong></div><div><span>Preferred Date</span><strong>{preferredDate||"—"}</strong></div><div><span>Preferred Time</span><strong>{preferredTime?.slice(0,5)||"—"}</strong></div><div><span>Room Access Permission</span><strong>{extra.room_access_permission?.toUpperCase()||"Not set"}</strong></div></section>
  ...
  {requiresAppointment&&complaint.status==="assigned"&&<section className="panel assignment-panel"><h3>Appointment Scheduling</h3><p className="subtle">Choose the final date and time independently from the student&apos;s preferences. Multiple maintenance jobs may use the same time slot.</p><form action={createAppointment.bind(null,id)} className="form-grid"><div className="field"><label>Appointment Date</label><input name="appointmentDate" type="date" required/></div><div className="field"><label>Appointment Time</label><select name="appointmentTime" defaultValue="" required><option value="" disabled>Choose a time slot</option>{appointmentTimeSlots.map(slot=><option key={slot.value} value={slot.value}>{slot.label}</option>)}</select></div><div className="field"><label>Assigned Maintenance Staff</label><select name="staffId" defaultValue={complaint.assignee?.id||""} required><option value="">Choose staff</option>{eligible?.map(x=><option key={x.id} value={x.id}>{x.full_name}</option>)}</select></div><div className="field field-wide"><label>Remarks</label><textarea name="remarks" rows={3}/></div><div><button className="button">Create Appointment</button></div></form></section>}
