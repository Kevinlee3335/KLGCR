...
export default async function ComplaintDetail({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<{error?:string;saved?:string;appointment?:string}> }) {
  ...
  const { data, error: complaintError } = await s.from("complaints").select("id,complaint_no,source,source_reference,photo_url,room_no,complainant_name,complainant_contact,category,description,priority,status,submitted_at,assigned_at,preferred_date,preferred_time,appointment_required,room_access_permission,block:blocks!block_id(id,code),assignee:profiles!assigned_to(id,full_name)").eq("id",id).maybeSingle();
  ...
  const extra = data as unknown as {preferred_date:string|null;preferred_time:string|null;appointment_required:boolean;room_access_permission:string|null};
  const appointments = appointmentRows ?? [];
  const requiresAppointment = extra.appointment_required ?? appointmentRequired(extra.room_access_permission);
  ...
  <section className="panel detail-grid" style={{marginBottom:18}}><div className="field-wide"><h3>Student Availability</h3><p className="subtle">These preferences are references only. The administrator sets any final appointment.</p></div><div><span>Student Name</span><strong>{complaint.complainant_name||"—"}</strong></div><div><span>Phone</span><strong>{complaint.complainant_contact||"—"}</strong></div><div><span>Preferred Date</span><strong>{extra.preferred_date||"—"}</strong></div><div><span>Preferred Time</span><strong>{extra.preferred_time?.slice(0,5)||"—"}</strong></div><div><span>Room Access Permission</span><strong>{extra.room_access_permission?.toUpperCase()||"Not set"}</strong></div></section>
  <aside className={`workflow-notice ${requiresAppointment?"workflow-warning":"workflow-success"}`}><strong>{requiresAppointment?"TENANT MUST BE PRESENT":"ROOM ACCESS GRANTED"}</strong><span>{requiresAppointment?"Appointment required before maintenance.":"Room access granted. Maintenance may enter without the tenant. No appointment required."}</span></aside>
