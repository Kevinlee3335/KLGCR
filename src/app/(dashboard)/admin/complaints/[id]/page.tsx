...
import { ConfirmedDefects } from "@/components/confirmed-defects";
...
export default async function ComplaintDetail({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<{error?:string;saved?:string;appointment?:string}> }) {
  ...
  const { data: defectRows } = await s.from("complaint_defects").select("id,item_group,item_name,issue_type,other_issue,exact_location,admin_confirmed_issue,maintenance_instruction,status").eq("complaint_id", id).order("created_at");
  ...
  {(source.photo_url||source.source_reference)&&<section className="panel" style={{marginBottom:18}}><h3>Google Form Source</h3>{source.source_reference&&<p><strong>Response:</strong> {source.source_reference}</p>}{source.photo_url&&<p><a className="text-link" href={source.photo_url} target="_blank" rel="noreferrer">Open Google Drive Photo / Attachment ↗</a></p>}</section>}<ComplaintForm blocks={blocks||[]} complaint={complaint} mode="review"/>{!["assigned","rejected","closed"].includes(complaint.status)&&<section className="panel assignment-panel review-assign-panel"><div className="review-assign-heading"><p className="eyebrow">Step 2</p><h3>Review defects & assign maintenance</h3><p className="subtle">Add all confirmed defects, then assign the appropriate maintenance staff below.</p></div><ConfirmedDefects complaintId={id} defects={(defectRows||[]) as never[]}/><div className="review-assign-divider"/><h3>Assign maintenance</h3><p className="subtle">Only active maintenance staff assigned to Block {complaint.block?.code} are available.</p><AssignmentForm complaintId={id} eligible={eligible||[]} requiresAppointment={requiresAppointment}/><RejectComplaintForm complaintId={id}/></section>}
  </AppShell>;
}
