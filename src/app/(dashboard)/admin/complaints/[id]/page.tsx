...
import { appointmentRequired, appointmentStatuses, titleCase } from "@/lib/appointments";
...
export default async function ComplaintDetail({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<{error?:string;saved?:string;appointment?:string}> }) {
  ...
  // Keep newly-migrated columns out of the required query. This lets Complaint
  // Review remain available while a deployment and its database migration are
  // rolling out, instead of turning a missing optional column into a false 404.
  ...
  const [{data:blocks},{data:eligible},{data:preferenceData,error:preferenceError},{data:appointmentRows,error:appointmentError}] = await Promise.all([
    ...
    s.from("complaints").select("preferred_date,preferred_time").eq("id",id).maybeSingle(),
  ...
  if (preferenceError) console.error("Preferred availability columns are not migrated yet", preferenceError.message);
  if (appointmentError) console.error("Optional appointment data could not be loaded", appointmentError.message);
  const legacy = data as unknown as { availability_date:string|null; availability_time:string|null; room_access_permission:string|null };
  const extra = {
    preferred_date: preferenceData?.preferred_date ?? legacy.availability_date,
    preferred_time: preferenceData?.preferred_time ?? legacy.availability_time,
    room_access_permission: legacy.room_access_permission,
  };
  ...
  <section className="panel detail-grid" style={{marginBottom:18}}><div className="field-wide"><h3>Student Availability</h3><p className="subtle">The preferred date and time do not schedule the visit. The administrator chooses the final appointment date and time.</p></div><div><span>Student Name</span><strong>{complaint.complainant_name||"—"}</strong></div><div><span>Phone</span><strong>{complaint.complainant_contact||"—"}</strong></div><div><span>Preferred Date</span><strong>{extra.preferred_date||"—"}</strong></div><div><span>Preferred Time</span><strong>{extra.preferred_time?.slice(0,5)||"—"}</strong></div><div><span>Room Access Permission</span><strong>{extra.room_access_permission?.toUpperCase()||"Not set"}</strong></div></section>
  <aside className={`workflow-notice ${requiresAppointment?"workflow-warning":"workflow-success"}`}><strong>{requiresAppointment?"TENANT MUST BE PRESENT":"ROOM ACCESS GRANTED"}</strong><span>{requiresAppointment?"Appointment Required.":"Maintenance staff may enter without the student. Appointment is NOT required."}</span></aside>
  ...
  {requiresAppointment&&complaint.status==="assigned"&&<section className="panel assignment-panel"><h3>Create Appointment</h3><p className="subtle">Choose the final date and time independently from the student&apos;s preferences.</p><form action={createAppointment.bind(null,id)} className="form-grid"><div className="field"><label>Appointment Date</label><input name="appointmentDate" type="date" required/></div><div className="field"><label>Appointment Time</label><input name="appointmentTime" type="time" required/></div><div className="field"><label>Assigned Staff</label><select name="staffId" defaultValue={complaint.assignee?.id||""} required><option value="">Choose staff</option>{eligible?.map(x=><option key={x.id} value={x.id}>{x.full_name}</option>)}</select></div><div className="field"><label>Appointment Status</label><select name="status" defaultValue="pending_confirmation">{appointmentStatuses.map(x=><option key={x} value={x}>{titleCase(x)}</option>)}</select></div><div className="field field-wide"><label>Remarks</label><textarea name="remarks" rows={3}/></div><div><button className="button">Create Appointment</button></div></form></section>}
