...
export default async function ComplaintDetail({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<{error?:string;saved?:string;appointment?:string}> }) {
  ...
  const [{data:blocks},{data:eligible},{data:availabilityData,error:availabilityError},{data:appointmentRows,error:appointmentError}] = await Promise.all([
    ...
    s.from("complaints").select("preferred_date,preferred_time,availability_date,availability_time,appointment_required,room_access_permission").eq("id",id).maybeSingle(),
  ...
  if (availabilityError) console.error("Optional availability data could not be loaded", availabilityError.message);
  if (appointmentError) console.error("Optional appointment data could not be loaded", appointmentError.message);
  const extra = availabilityData ?? {preferred_date:null,preferred_time:null,availability_date:null,availability_time:null,appointment_required:true,room_access_permission:null};
  ...
  // Room access is the workflow decision in V3. Derive the UI from the answer
  ...
  <section className="panel detail-grid" style={{marginBottom:18}}><div className="field-wide"><h3>Student Preferences (Reference Only)</h3><p className="subtle">The preferred date and time do not schedule the visit. The administrator chooses the final appointment date and time.</p></div><div><span>Student Name</span><strong>{complaint.complainant_name||"—"}</strong></div><div><span>Phone</span><strong>{complaint.complainant_contact||"—"}</strong></div><div><span>Preferred Date</span><strong>{preferredDate||"—"}</strong></div><div><span>Preferred Time</span><strong>{preferredTime?.slice(0,5)||"—"}</strong></div><div><span>Room Access Permission</span><strong>{extra.room_access_permission?.toUpperCase()||"Not set"}</strong></div></section>
  <aside className={`workflow-notice ${requiresAppointment?"workflow-warning":"workflow-success"}`}><strong>{requiresAppointment?"NO — APPOINTMENT REQUIRED":"YES — NO APPOINTMENT REQUIRED"}</strong><span>{requiresAppointment?"Room access was not granted. The administrator must set the final appointment.":"Room access was granted, so maintenance may proceed without an appointment."}</span></aside>
