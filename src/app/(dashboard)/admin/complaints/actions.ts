...
import { appointmentRequired, appointmentTimeValues, validateAppointmentSelection } from "@/lib/appointments";
...
export async function assignComplaint(id:string,data:FormData){
  ...
  const selection=validateAppointmentSelection(data.get("appointmentDate"),data.get("appointmentTime"),needsAppointment);
  if(!selection.success)redirect(`/admin/complaints/${id}?error=${encodeURIComponent(selection.error)}`);
  ...
  if(selection.appointment){
    ...
    const {error:appointmentError}=await s.from("appointments").insert({complaint_id:id,job_id:job.id,appointment_date:selection.appointment.appointmentDate,appointment_time:selection.appointment.appointmentTime,assigned_staff:staffId,remarks:null,status:"pending_confirmation",created_by:actor.id});
