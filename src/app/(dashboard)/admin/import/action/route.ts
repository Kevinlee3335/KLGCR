...
export async function POST(request: Request) {
  ...
  for(let index=1;index<rows.length;index++){
    ...
    const block=blockCode(value(rec,["Block","Building"])); const room=value(rec,["Room","Room No","Room Number"]); const category=value(rec,["Category","Complaint Category","Type"]); const description=value(rec,["Description","Complaint","Issue","Problem"]);
