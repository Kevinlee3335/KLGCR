...
export default async function ImportPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  return <AppShell profile={profile} title="Google Import">
    ...
    {isAdmin && <section className="panel page-narrow"><h3>Import response rows</h3><p className="subtle">Required columns: Block, Room, Category, Description, Room Availability (Date), Room Availability (Time), and Request for Room Access Due to Tenant&apos;s Unavailability. Google Drive photo links can be stored in Photo URL.</p><form action="/admin/import/action" method="post" encType="multipart/form-data"><div className="field"><label>CSV file from Google Sheet</label><input type="file" name="file" accept=".csv,text/csv,text/plain"/></div><div className="field"><label>Or paste Google Sheet rows</label><textarea name="pasted" rows={10} placeholder={'Block,Room,Category,Description,Room Availability (Date),Room Availability (Time),Request for Room Access Due to Tenant\'s Unavailability\nA,A101,Plumbing,Water leaking,9/18/2026,2:30 PM,YES'}/></div><button className="button" type="submit">Import as New Complaints</button></form></section>}
