...
export async function createCalendarEvent(data: FormData) {
  ...
  const startsAt = dateTime(data.get("date"), data.get("startTime"));
  const endsAt = data.get("endTime") ? dateTime(data.get("date"), data.get("endTime")) : null;
  ...
  if (actor.role === "admin") {
    audience = data.get("audience") === "all_staff" ? "all_staff" : "individual";
  ...
  const { error } = await db.from("calendar_events").insert({ title, notes: notes || null, starts_at: startsAt, ends_at: endsAt, audience, assigned_to: assignedTo, created_by: actor.id });
